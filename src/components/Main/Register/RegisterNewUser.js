...
const RegisterNewUser = (props) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (emailUser) {
            ...
            if (state.pass === state.auth_pass) {
                props.hendleVerificationUser(state
                        .status = "user",
                    state.name,
                );
                history.push("./");
    ...
    return (
        <form className="form_auth_style" id="regformtwo" onSubmit={handleSubmit}>
            <label>Введите ваше имя</label>
            <input type="text" name="name" placeholder="Имя" required id="regname"
                   onChange={handleInputChange}
                   value={state.name || ''}
            />
            <label>Введите вашу фамилию</label>
            <input type="text" name="lastname" placeholder="Фамилия" required id="reglastname"
                   onChange={handleInputChange}
                   value={state.lastname || ''}
            />
            <label style={state.passLabelError}>{state.mesEmail ? state.mesEmail : "Введите ваш email"}</label>
            <input type="email" name="email" placeholder="@email" required id="auth_email"
                   onChange={handleInputChange}
                   value={state.email || ''}
                   style={state.passInputError || ''}
            />
            <label style={state.nonCheckLabel}>{state.mesPass ? state.mesPass : "Введите Ваш пароль"}</label>
            <input type="password" name="pass" placeholder="password" required id="reg_pass"
                   minLength="7"
                   onChange={handleInputChange}
                   value={state.pass || ''}
            ...
            <input type="password" name="auth_pass" placeholder="password" required id="reg_passtwo"
                   ...
                   style={state.nonCheckInput || ''}
            />
            <button className="form_auth_button" name="form_auth_submit"
                    id="reg_btn">Регистрация
            </button>
            <button className="form_auth_button" type="reset" onClick={handleClickRegister}>Очистить</button>
        </form>
    )
}
